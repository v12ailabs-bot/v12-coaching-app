...
// Call" opens the coach's own Calendly scheduled-events dashboard (who's
// booked, for when) — not the public booking page (that's what clients get,
// see WeeklyCheckin in App.jsx; sending the coach there would just let them
// book a slot with themselves) — and the two less-common actions (Generate
// AI Program, Update nutrition plan) sit behind "More Actions" instead of
...
export function ClientQuickActionsRail({ onGenerateProgram, onUpdateNutrition, onUpdateProgramPhase, onSendMessage, onAddNote }) {
  ...
  const handlers = {
    ...
    call: () => window.open(V12_CALENDLY_DASHBOARD_URL, "_blank", "noopener"),
