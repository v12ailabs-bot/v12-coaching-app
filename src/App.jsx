...
<div style={{ color: “var(–muted)”, fontSize: 13, marginBottom: 20 }}>Log your first daily check-in to start tracking progress.</div>
...
useEffect(() => {
supabase.from(“daily_checkins”).select(”*”).eq(“client_id”, profile.id).eq(“date”, today()).maybeSingle()
...
setLoading(true);
const entry = { client_id: profile.id, date: today(), …form, weight: parseFloat(form.weight) || null };
...
<div className="page-sub">{today()}{existing ? “ · Updating today’s entry” : “”}</div>
<div className="card">
...
<div />
{[[“sleep”,“😴 Sleep Quality”,1,10,””], [“energy”,“⚡ Energy Level”,1,10,””], [“mood”,“🧠 Mood”,1,10,””], [“water”,“💧 Water (glasses)”,0,16,” glasses”]].map(([k,label,min,max,sfx]) => (
<div key={k}>
<div className="slider-label"><span>{label}</span><span className="slider-val">{form[k]}{sfx || “/10”}</span></div>
...
<div className="page-sub">Week of {weekStart}{existing ? “ · Updating this week” : “”}</div>
<div className="card">
...
<div className="grid-2">
<CC title="Goal Progress" sub="Weekly self-assessment %">
<ResponsiveContainer width="100%" height="100%">
...
<tr key={i}>
<td>{row.date}</td>
<td>{row.weight ? `${row.weight} lbs` : "BW"}</td>
<td>{row.reps||"—"}</td>
<td>{row.sets}</td>
<td>{row.time||"—"}</td>
</tr>
...
{clients.length === 0 && <div style={{ color:“var(–muted)”, fontSize:13, padding:“20px 0” }}>No clients yet. Have them create an account using the app link.</div>}
{clients.map((c,i) => (
...
<div>{counts[c.id]||0} check-ins</div>
</div>
</div>
))}
</div>
...
{clients.length===0 && <div style={{ color:"var(--muted)", fontSize:13 }}>No clients yet. Share the app with your clients and have them create an account.</div>}
</div>
...
<input type="text" value={newEx.name} onChange={e => setNewEx(p=>({...p,name:e.target.value}))} placeholder="e.g. Squat" />
</div>
<div className="field">
<label>Category</label>
<input type="text" value={newEx.category} onChange={e => setNewEx(p=>({...p,category:e.target.value}))} placeholder="e.g. Lower Body" />
</div>
<div className="field">
<label>Type</label>
<div className="radio-group">
